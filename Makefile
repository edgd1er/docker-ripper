...
build: ## build image
	...
	docker buildx build --load --progress plain --build-arg FDKVERSION="${FDKVERSION}" --build-arg aptCacher="${aptcacher}" --build-arg PREFIX="/usr/local" --build-arg=MKVVERSION="${MKVVERSION}" --platform amd64 -f ./Dockerfile .
